...
class Tank_auth {
    ...
    function generator_user($password) {
        return $this->password_generator($password);
    }

    function password_generator($password) {
        $hasher = new PasswordHash(
                $this->ci->config->item('phpass_hash_strength', 'tank_auth'), $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
        return $hasher->HashPassword($password);
    }

    function checking_password($password, $db_password) {
        $hasher = new PasswordHash(
                $this->ci->config->item('phpass_hash_strength', 'tank_auth'), $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
